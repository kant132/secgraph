...
- 如果已分析调用链但还没验证（无 poc_result）→ verify
- 如果所有 findings 都有 poc_result → FINISH
- 如果 verify 生成了 second_payload 需要重新验证 → verify
- 如果 verify 失败需要进一步探索代码 → trace
- 重要：如果 discovery 还没审完（work_list 有剩余，audit_index < len(work_list)），
  且当前有未分析调用链的 findings，优先 trace 分析调用链
  trace 完后如果有剩余未审，回 discovery 继续审计下一批

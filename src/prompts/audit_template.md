...
3、得出结论
4、以下情况不是漏洞，不要误报：
   - PreparedStatement / 预编译语句 + ? 占位符 = 安全，不是 SQLi
   - MyBatis #{} = 安全，不是 SQLi（只有 ${} 才是）
   - indexOf / contains / equals 等字符串操作 = 安全，不是注入
   - 参数经过白名单/正则校验后才使用 = 已消毒，不是漏洞
   - 只要看代码逻辑确认是安全的，不要报
5、关键规则：如果分析后确认是安全的（如用了参数化查询、有消毒方法），
   就不要报为漏洞，返回 {}。只有确认存在真实可利用的漏洞时才报。
